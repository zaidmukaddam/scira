...
export function FlightTracker({ data }: FlightTrackerProps) {
	...
	const mapStatus = (
		status: string,
	): "LANDED" | "DEPARTING ON TIME" | "DELAYED" | "SCHEDULED" => {
	...
	return (
		<T id="components.flight_tracker.4">
				...
				<CardContent className="p-4 md:p-8">
						...
						<div className="flex items-center gap-3">
							...
							<div>
								<h2 className="text-xl md:text-2xl font-bold tracking-tight">
									<Var>{flightInfo.flightNumber}</Var>
								</h2>
								<p className="text-sm text-neutral-500 dark:text-neutral-400">
									<Var>{flight.airline.name}</Var>
								</p>
							</div>
						</div>
						<Badge
						...
						<div className="flex flex-col md:flex-row gap-6 md:gap-8 md:items-center">
							...
							<div className="flex-1 min-w-0">
								<div className="text-2xl md:text-3xl font-mono font-bold mb-2 truncate">
									<Var>{flightInfo.departure.code}</Var>
								</div>
								<div className="space-y-0.5 md:space-y-1">
									<p className="font-medium text-sm truncate">
										<Var>{flightInfo.departure.airport}</Var>
									</p>
									<p className="text-lg md:text-xl font-bold">
										<Var>{flightInfo.departure.time}</Var>
									</p>
									<p className="text-xs text-neutral-500">
										<Var>{flightInfo.departure.date}</Var>
									</p>
								</div>
							</div>
							...
							<div className="md:hidden relative w-[97%] h-8 flex items-center">
								...
								{/* Progress Bar */}
								<div
									className={`absolute h-1 top-1/2 -translate-y-1/2 bg-blue-500 rounded-full transition-all duration-1000 ${
										flightInfo.status === "LANDED"
											? "w-full"
											: flightInfo.status === "DEPARTING ON TIME"
												? "w-[5%]"
												: "w-1/2"
									}`}
								/>
							...
							<div className="flex-1 min-w-0 md:text-right">
								<div className="text-2xl md:text-3xl font-mono font-bold mb-2 truncate">
									<Var>{flightInfo.arrival.code}</Var>
								</div>
								<div className="space-y-0.5 md:space-y-1">
									<p className="font-medium text-sm truncate">
										<Var>{flightInfo.arrival.airport}</Var>
									</p>
									<p className="text-lg md:text-xl font-bold">
										<Var>{flightInfo.arrival.time}</Var>
									</p>
									<p className="text-xs text-neutral-500">
										<Var>{flightInfo.arrival.date}</Var>
									</p>
								</div>
							</div>
						</div>
						...
						<div className="space-y-3 max-w-full">
							<Var>
								{flightInfo.departure.terminal && (
									<T id="components.flight_tracker.0">
										<div className="flex items-center gap-2 overflow-hidden">
											<Terminal className="h-4 w-4 shrink-0 text-neutral-500" />
											<span className="text-sm truncate">
												Terminal <Var>{flightInfo.departure.terminal}</Var>
											</span>
										</div>
									</T>
								)}
							...
							<Var>
								{flightInfo.departure.gate && (
									<T id="components.flight_tracker.1">
										<div className="flex items-center gap-2 overflow-hidden">
											<div className="h-4 w-4 shrink-0 rounded bg-blue-500/10 flex items-center justify-center text-[10px] text-blue-600">
												G
											</div>
											<span className="text-sm truncate">
												Gate <Var>{flightInfo.departure.gate}</Var>
											</span>
										</div>
									</T>
								)}
						...
						<div className="space-y-3 max-w-full">
							<Var>
								{flightInfo.arrival.terminal && (
									<T id="components.flight_tracker.2">
										<div className="flex items-center gap-2 overflow-hidden">
											<Terminal className="h-4 w-4 shrink-0 text-neutral-500" />
											<span className="text-sm truncate">
												Terminal <Var>{flightInfo.arrival.terminal}</Var>
											</span>
										</div>
									</T>
								)}
							...
							<Var>
								{flightInfo.arrival.gate && (
									<T id="components.flight_tracker.3">
										<div className="flex items-center gap-2 overflow-hidden">
											<div className="h-4 w-4 shrink-0 rounded bg-blue-500/10 flex items-center justify-center text-[10px] text-blue-600">
												G
											</div>
											<span className="text-sm truncate">
												Gate <Var>{flightInfo.arrival.gate}</Var>
											</span>
										</div>
									</T>
								)}
					...
					<div className="mt-4 md:mt-6 flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-3 text-sm text-neutral-500">
						<div className="flex items-center gap-2">
							...
							<span>
								Flight duration: <Var>{flightInfo.duration}</Var>
							</span>
						</div>
						<span className="hidden md:inline text-neutral-300">•</span>
						<span>
							Last updated: <Var>{flightInfo.lastUpdated}</Var>
						</span>
					</div>
				</CardContent>
			</Card>
