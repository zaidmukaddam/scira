...
import { MagicWandIcon as WandIcon } from "@hugeicons/core-free-icons";
...
export function CorrectionLibellerLoadingState({ size = 80, duration = 5, className, ...props }: CorrectionLibellerLoadingStateProps) {
  return (
    <Card
      ...
      role="status"
      aria-live="polite"
      aria-label="Correction des libellés en cours"
      {...props}
    >
      <BorderTrail
        className={cn(
          ...
          <div className="flex items-center gap-3">
            <div className="relative h-10 w-10 rounded-full flex items-center justify-center bg-[#EDEAE7]/60 dark:bg-[#3C3732]/40">
              <BorderTrail
                ...
                transition={{ repeat: Infinity, duration, ease: "linear" }}
              />
              <HugeiconsIcon icon={WandIcon} size={20} color="currentColor" strokeWidth={2} className={cn("text-[#70665D] dark:text-[#C7C0B9]")} />
            </div>
