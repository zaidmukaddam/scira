...
const Messages: React.FC<MessagesProps> = ({
	...
	const renderPart = (
		...
		if (part.type === "reasoning") {
			...
			if (
				isComplete &&
				reasoningTimings[sectionKey] &&
				!reasoningTimings[sectionKey].endTime
			) {
		...
		switch (part.type) {
			...
			case "reasoning": {
				...
				const parallelTool = hasParallelToolInvocation
					? (parts.find((p: MessagePart) => p.type === "tool-invocation")
							?.toolInvocation?.toolName ?? null)
					: null;
	...
	useEffect(() => {
		...
		const activeReasoningSections = Object.entries(reasoningTimings).filter(
			([_, timing]) => !timing.endTime,
		);
			...
			{memoizedMessages.map((message, index) => (
				...
				>
					<Message
						...
						suggestedQuestions={
							index === memoizedMessages.length - 1 ? suggestedQuestions : []
						}
					/>
				</div>
			))}
