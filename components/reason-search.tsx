		...
		<div id={id} className="group">
			...
			>
				...
				>
					<div className="space-y-0.5 min-w-0 flex-1">
						<div className="flex items-center gap-2 min-w-0">
							<span className="text-sm font-medium truncate">
								{update.title ||
									(update.type === "plan" ? (
										<T id="components.reason_search.0">{"Research Plan"}</T>
									) : update.type === "x" ? (
										<T id="components.reason_search.1">{"X Search"}</T>
									) : (
										<T id="components.reason_search.2">{"Analysis"}</T>
									))}
							</span>
							{update.type === "plan" && update.plan && (
								<T id="components.reason_search.3">
									<span className="text-xs text-neutral-500 shrink-0">
										(<Var>{update.plan.search_queries.length}</Var> queries,{" "}
										<Var>{update.plan.required_analyses.length}</Var> analyses
										<Var>
					...
					<ChevronRight
						className={cn(
							"h-4 w-4 text-neutral-400 shrink-0 ml-2 transition-transform",
											...
											<div className="flex items-start gap-2">
												...
												<div>
													<div className="flex items-center gap-2 flex-wrap">
														...
														{query.source === "web" && (
															<T id="components.reason_search.8">
																<Badge
																	variant="secondary"
																	className="text-[10px]"
																>
														...
														{query.source === "academic" && (
															<T id="components.reason_search.9">
																<Badge
																	variant="secondary"
																	className="text-[10px]"
																>
														...
														{query.source === "both" && (
															<T id="components.reason_search.10">
																<Badge
																	variant="secondary"
																	className="text-[10px]"
																>
														...
														{query.source === "x" && (
															<T id="components.reason_search.11">
																<Badge
																	variant="secondary"
														...
														{query.source === "all" && (
															<T id="components.reason_search.12">
																<Badge
																	variant="secondary"
																	className="text-[10px] flex items-center gap-0.5"
																>
										...
										<div className="p-3 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-black">
											<div className="flex items-center gap-3">
												...
												<p className="text-xs text-neutral-500">
													{update.type === "x" ? (
														<T id="components.reason_search.4">
															{"Searching X..."}
														</T>
													) : update.type === "web" ? (
														<T id="components.reason_search.5">
															{"Searching the web..."}
														</T>
													) : update.type === "academic" ? (
														<T id="components.reason_search.6">
															{"Searching academic sources..."}
														</T>
													) : (
														<T id="components.reason_search.7">
...
const AllSourcesView = ({
	...
	if (isDesktop) {
		return (
			<T id="components.reason_search.13">
				<Dialog>
					...
					>
						<DialogHeader>
							<DialogTitle className="flex items-center gap-2">
								<Var>{type === "web" && <FileText className="h-4 w-4" />}</Var>
								<Var>
									{type === "academic" && <BookA className="h-4 w-4" />}
								</Var>
								<Var>{type === "x" && <XLogo className="h-4 w-4" />}</Var>
								<Var>{title}</Var>
							</DialogTitle>
						</DialogHeader>
	...
	return (
		<T id="components.reason_search.14">
			<Drawer>
				...
				<DrawerContent className="h-[85vh]">
					<DrawerHeader>
						<DrawerTitle className="flex items-center gap-2">
							<Var>{type === "web" && <FileText className="h-4 w-4" />}</Var>
							<Var>{type === "academic" && <BookA className="h-4 w-4" />}</Var>
							<Var>{type === "x" && <XLogo className="h-4 w-4" />}</Var>
							<Var>{title}</Var>
						</DrawerTitle>
					</DrawerHeader>
...
const AnimatedTabContent = ({
	...
	value,
	selected,
}: { children: React.ReactNode; value: string; selected: string }) => (
	<motion.div
		role="tabpanel"
		...
		<div className="space-y-8">
					...
					<div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
						<div className="flex items-center gap-2">
							<h3 className="text-sm font-medium">
								{isComplete ? (
									<T id="components.reason_search.15">{"Research Complete"}</T>
								) : (
									<T id="components.reason_search.16">{"Research Progress"}</T>
								)}
							</h3>
							{isComplete ? (
								<T id="components.reason_search.17">
									<Badge
										variant="secondary"
								...
								showRunningIndicators && (
									<T id="components.reason_search.18">
										<Badge
											variant="secondary"
			...
			{finalSynthesisDone &&
				(sourceGroups.web.length > 0 ||
					...
					<T id="components.reason_search.27">
								...
								<div className="relative mt-4">
												...
												<div className="space-y-3">
													<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
														...
														{sourceGroups.web.length > 3 && (
															<T id="components.reason_search.19">
																<button
																	onClick={() =>
																		document
																...
																>
																	...
																	<span className="text-xs text-neutral-600 dark:text-neutral-400 group-hover:text-neutral-700 dark:group-hover:text-neutral-300">
																		Show{" "}
																		<Var>{sourceGroups.web.length - 3}</Var>{" "}
																		More Sources
																	</span>
																</button>
															</T>
												...
												<div className="space-y-3">
													<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
														...
														{sourceGroups.academic.length > 3 && (
															<T id="components.reason_search.20">
																<button
																	onClick={() =>
																		document
																			.getElementById(
																				"show-all-academic-sources",
																			)
																			?.click()
																	}
																	className="flex items-center justify-center gap-2 p-2.5 rounded-lg border border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-colors group"
												...
												<div className="space-y-3">
													<div className="relative rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-black">
														...
														{sourceGroups.x.length > 2 && (
															<T id="components.reason_search.21">
																<div className="absolute bottom-0 inset-x-0 flex items-center justify-center pb-4 pt-12 bg-linear-to-t from-white via-white to-transparent dark:from-black dark:via-black rounded-b-lg">
																	...
																	>
																		...
																		<span className="text-xs text-neutral-600 dark:text-neutral-400 group-hover:text-neutral-700 dark:group-hover:text-neutral-300">
																			Show all{" "}
																			<Var>{sourceGroups.x.length}</Var> X posts
																		</span>
																	</button>
																</div>
															</T>
									...
									<AnimatedTabContent value="analysis" selected={selectedTab}>
										<Var>
											...
											updates.some(
												(u) =>
													u.id === "final-synthesis" && u.status === "running",
											) ? (
												<div className="space-y-2">
													...
													{sourceGroups.analysis.find(
														...
														<T id="components.reason_search.23">
															<Accordion
																type="single"
																collapsible
																className="bg-white dark:bg-black rounded-lg border border-border"
																		...
																		<div className="grid gap-1.5">
																			<Var>
																				{updates.some(
																					...
																					<T id="components.reason_search.22">
													...
													{sourceGroups.analysis.find(
														...
														<T id="components.reason_search.26">
															<Accordion
																type="single"
																collapsible
																className="bg-white dark:bg-black rounded-lg border border-border"
																		...
																		<div className="grid gap-2.5">
																			...
																			<Var>
																				{sourceGroups.analysis.find(
																					...
																					<T id="components.reason_search.24">
																						<div className="space-y-2">
																							<h4 className="text-[11px] font-medium text-neutral-600 px-1">
																								Remaining Uncertainties
																							</h4>
																			...
																			<Var>
																				{sourceGroups.analysis.find(
																					...
																					<T id="components.reason_search.25">
																						<div className="space-y-2">
																							<h4 className="text-[11px] font-medium text-neutral-600 px-1">
																								Recommendations
																							</h4>
