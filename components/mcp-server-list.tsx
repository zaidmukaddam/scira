...
export const MCPServerList: React.FC<MCPServerListProps> = ({
	...
	if (!servers || servers.length === 0) {
		return (
			<T id="components.mcp_server_list.0">
	...
	return (
		<T id="components.mcp_server_list.2">
			<div className="space-y-3">
				<div className="px-1">
					<p className="text-xs text-neutral-500 dark:text-neutral-400">
						<Var>{servers.length}</Var> server
						<Var>
							{servers.length !== 1 ? (
								<T id="components.mcp_server_list.1">{"s"}</T>
							) : (
								""
							)}
						</Var>{" "}
									...
									<div className="p-3.5">
										...
										<div className="flex flex-wrap gap-1.5">
											...
											{server.deploymentUrl && (
												<T id="components.mcp_server_list.3">
													<TooltipProvider>
														<Tooltip>
															<TooltipTrigger asChild>
																<Badge
										...
										{server.useCount && parseInt(server.useCount) > 0 && (
											<T id="components.mcp_server_list.5">
												<div className="flex justify-between text-[10px] text-neutral-500 mt-2.5">
													<span>
														Usage: <Var>{server.useCount}</Var>
													</span>
													<Var>
														{server.createdAt && (
															<T id="components.mcp_server_list.4">
																<span>
																	Added:{" "}
																	<Var>
																		{new Date(
