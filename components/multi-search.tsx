...
const SearchLoadingState = ({
	queries,
	annotations,
}: { queries: string[]; annotations: QueryCompletion[] }) => {
	...
	return (
		<T id="components.multi_search.0">
						...
						<AccordionTrigger className="py-3 px-4 bg-white dark:bg-neutral-900 rounded-lg hover:no-underline border border-neutral-200 dark:border-neutral-800 data-[state=open]:rounded-b-none">
							<div className="flex items-center justify-between w-full">
								<div className="flex items-center gap-2">
									...
									<h2 className="font-medium text-left text-sm">
										Sources Found
									</h2>
								</div>
								<div className="flex items-center gap-2">
									...
									>
										...
										<Var>{totalResults || "0"}</Var> Results
									</Badge>
								</div>
							</div>
						</AccordionTrigger>
...
const ImageGrid = ({ images, showAll = false }: ImageGridProps) => {
	...
	return (
		<div>
					...
					>
						...
						{image.description && (
							<div className="absolute inset-0 bg-linear-to-t from-black/70 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 p-2 flex items-end">
								<p className="text-xs text-white line-clamp-2 w-full">
									{image.description}
								</p>
							</div>
						)}
						{!showAll && hasMore && index === displayImages.length - 1 && (
							<T id="components.multi_search.3">
								<div className="absolute inset-0 flex items-center justify-center">
									<Badge
										variant="secondary"
										className={cn(
									...
									>
										+<Var>{images.length - displayImages.length}</Var> more
									</Badge>
								</div>
							</T>
						)}
			...
			{isDesktop ? (
				<T id="components.multi_search.1">
						...
						<DialogContent className="max-w-4xl! w-[90vw]! h-[65vh]! p-0 overflow-hidden border-none shadow-xl [&>button]:hidden">
							<div className="relative w-full h-full rounded-lg overflow-hidden bg-white dark:bg-neutral-900">
								<div className="absolute top-0 left-0 right-0 z-50 p-2 flex justify-between items-center">
									<Badge
										variant="secondary"
										className="rounded-full bg-white/90 dark:bg-neutral-800/90 text-neutral-700 dark:text-neutral-300 border border-neutral-200 dark:border-neutral-700"
									>
										<span className="text-xs font-normal">
											<Var>{selectedImage + 1}</Var> of{" "}
											<Var>{images.length}</Var>
										</span>
									</Badge>
									<Button
										variant="outline"
				...
				<T id="components.multi_search.2">
...
const MultiSearch: React.FC<{
	...
	return (
		<T id="components.multi_search.4">
			<div className="w-full space-y-3">
				<Accordion
					type="single"
					collapsible
				...
				<Var>{allImages.length > 0 && <ImageGrid images={allImages} />}</Var>
			</div>
		</T>
	);
};
