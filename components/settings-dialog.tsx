...
import {
  ...
  updateUserSuggestedQuestionsAction,
} from "@/app/actions";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={isMobile ? "space-y-2" : "space-y-3"}>
        <div
          className={cn(
            "bg-muted/50 rounded-lg space-y-3",
            isMobile ? "p-3" : "p-4",
          )}
        >
          <div>
...
function UsageBarChart({
  ...
  const chartConfig = {
    count: {
      label: "Messages",
      color:
        resolvedTheme === "dark" ? "hsl(200 100% 50%)" : "hsl(220 70% 50%)", // Bright cyan in dark, blue in light
    },
  };
  ...
  const barColor =
    resolvedTheme === "dark" ? "hsl(200 100% 50%)" : "hsl(220 70% 50%)";
      ...
      <ChartContainer config={chartConfig} className="w-full h-full">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart
            ...
            margin={
              isMobile
                ? { top: 10, right: 0, left: 10, bottom: 5 }
                : { top: 10, right: 5, left: 25, bottom: 10 }
            }
          >
            <XAxis
              dataKey="day"
            ...
            <YAxis
              ...
              domain={[0, yAxisMax]}
              ticks={Array.from(
                { length: Math.min(4, Math.floor(yAxisMax / 5) + 1) },
                (_, i) => i * 5,
              )}
  ...
  return (
    ...
    >
      ...
      <div className={cn("grid grid-cols-2", isMobile ? "gap-2" : "gap-3")}>
        <div
          className={cn(
            "bg-muted/50 rounded-lg space-y-1",
            isMobile ? "p-2.5" : "p-3",
          )}
        >
          ...
          {usageLoading ? (
            <Skeleton
              className={cn(
                "font-semibold",
                isMobile ? "text-base h-4" : "text-lg h-5",
              )}
            />
          ) : (
            <div
      ...
      {!isProUser && (
        <div className={isMobile ? "space-y-2" : "space-y-3"}>
          <div
            className={cn(
              "bg-muted/30 rounded-lg space-y-2",
              isMobile ? "p-2.5" : "p-3",
            )}
          >
            {usageLoading ? (
      ...
      {!usageLoading && (
        <div className={cn("space-y-2", isMobile && !isProUser ? "pb-4" : "")}>
          ...
          <div
            className={cn(
              "bg-muted/50 dark:bg-card rounded-lg overflow-hidden",
              isMobile ? "p-2" : "p-3",
            )}
          >
            {historicalLoading ? (
          ...
          >
            ...
            >
              {isManagingSubscription ? (
                <Loader2
                  className={isMobile ? "h-3 w-3 mr-1.5" : "h-3.5 w-3.5 mr-2"}
                />
              ) : (
                <ExternalLink
                  className={isMobile ? "h-3 w-3 mr-1.5" : "h-3.5 w-3.5 mr-2"}
...
function CustomInstructionsSection({
  ...
  const [isSuggestedQuestionsEnabled, setIsSuggestedQuestionsEnabled] =
    useState(user?.suggestedQuestions ?? true);
  ...
  const handleSuggestedQuestionsToggle = async (checked: boolean) => {
    setIsUpdatingPreference(true);
    try {
      const result = await updateUserSuggestedQuestionsAction(checked);
      if (result.success) {
        setIsSuggestedQuestionsEnabled(checked);
        toast.success(
          checked
        ...
        setIsSuggestedQuestionsEnabled(!checked); // Revert on error
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <Label htmlFor="enable-suggestions" className="text-sm font-medium">
            Enable Suggested Questions
          </Label>
          <p className="text-xs text-muted-foreground mt-1">
            Show follow-up questions after AI responses to help continue the
            conversation
          </p>
        </div>
        <Switch
          id="enable-suggestions"
          checked={isSuggestedQuestionsEnabled}
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Toggle to enable or disable custom instructions for your
            conversations
          </p>
        </div>
        <Switch
        ...
        {customInstructionsLoading ? (
          ...
          <div className="p-2.5 bg-muted/30 rounded-lg">
            <p className="text-[10px] text-muted-foreground">
              Last updated:{" "}
              {new Date(customInstructions.updatedAt).toLocaleDateString()}
            </p>
          </div>
        ) : null}
      </div>
...
function MemoriesSection() {
  ...
  const totalMemories =
    searchQuery.trim() && searchResults
      ? searchResults.total
      : memoriesData?.pages.reduce(
          (acc, page) => acc + page.memories.length,
          0,
        ) || 0;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="!max-w-4xl !w-full max-h-[85vh] !p-0 gap-0 overflow-hidden">
        <DialogHeader className="p-4 !m-0">
          <DialogTitle className="text-xl font-medium tracking-normal">
            Settings
          </DialogTitle>
        </DialogHeader>
