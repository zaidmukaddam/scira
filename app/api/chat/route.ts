...
async function executeStockChartTool({ code, title, icon }: { code: string; title: string; icon: string }) {
...
async function executeCurrencyConverterTool({ from, to }: { from: string; to: string }) {
    const code = `
...
async function executeXSearchTool({
    query,
    startDate,
    endDate,
}: {
...
async function executeTMDBSearchTool({ query }: { query: string }) {
...
async function executeTrendingMoviesTool() {
    const TMDB_API_KEY = serverEnv.TMDB_API_KEY;
    const TMDB_BASE_URL = 'https://api.themoviedb.org/3';
...
async function executeTrendingTVTool() {
    const TMDB_API_KEY = serverEnv.TMDB_API_KEY;
    const TMDB_BASE_URL = 'https://api.themoviedb.org/3';
...
async function executeAcademicSearchTool({ query }: { query: string }) {
    try {
        const exa = new Exa(serverEnv.EXA_API_KEY as string);
...
async function executeYouTubeSearchTool({ query, no_of_results }: { query: string; no_of_results: number }) {
    try {
        const exa = new Exa(serverEnv.EXA_API_KEY as string);
...
async function executeRetrieveTool({ url }: { url: string }) {
...
async function executeGetWeatherDataTool({ lat, lon }: { lat: number; lon: number }) {
...
async function executeCodeInterpreterTool({ code, title, icon }: { code: string; title: string; icon: string }) {
    console.log('Code:', code);
    console.log('Title:', title);
    console.log('Icon:', icon);
...
async function executeFindPlaceTool({ query, coordinates }: { query: string; coordinates: number[] }) {
    try {
...
async function executeTextSearchTool({
    query,
    location,
    radius,
}: {
...
async function executeNearbySearchTool({
    location,
    latitude,
    longitude,
    type,
...
async function executeTrackFlightTool({ flight_number }: { flight_number: string }) {
    try {
        const response = await fetch(
    ...
    const result = streamText({
        ...
        tools: {
            ...
            currency_converter: tool({
                ...
                execute: executeCurrencyConverterTool,
            }),
            web_search: tool({
            ...
            academic_search: tool({
                ...
                execute: executeAcademicSearchTool,
            }),
            youtube_search: tool({
                ...
                execute: executeYouTubeSearchTool,
            }),
            retrieve: tool({
            ...
            get_weather_data: tool({
                ...
                execute: executeGetWeatherDataTool,
            }),
            ...
            nearby_search: tool({
                ...
                execute: executeNearbySearchTool,
            }),
            track_flight: tool({
                ...
                execute: executeTrackFlightTool,
            }),
        },
        onChunk(event) {
